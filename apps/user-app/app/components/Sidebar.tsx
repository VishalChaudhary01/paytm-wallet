                    ...
                    <SheetContent side="left" className="w-56 bg-sky-50 py-16">
                         <SheetTitle>
                              {sidebarLinks.map((link) => (
                                   <Link href={link.href} className="flex gap-2 text-base font-medium text-gray-1 hover:bg-blue-3 hover:text-white transition duration-200 ease-in-out px-4 py-2 rounded-md w-full">
                                        {link.icon}
                                        <span>{link.name}</span>
                                   </Link>
                              ))}   
